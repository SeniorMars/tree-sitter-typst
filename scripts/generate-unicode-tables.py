...
JS_OUT = Path(os.environ.get(
    "TREE_SITTER_TYPST_UNICODE_RANGES_OUT",
    ROOT / "grammar" / "unicode-ranges.js",
))
...
def js_ranges(tables: dict[str, list[tuple[int, int]]]) -> str:
    entries = []
    for name in REQUIRED_TABLES:
        ranges = normalize_ranges(name, tables[name])
        body = ", ".join(f"[0x{lo:X}, 0x{hi:X}]" for lo, hi in ranges)
        entries.append(f"  {name}: Object.freeze([{body}])")
    return ",\n".join(entries)
...
def generate_outputs() -> tuple[str, str]:
    ...
    tables = {
        "XID_START_RANGES": xid_start,
        "XID_CONTINUE_RANGES": xid_continue,
        "NUMBER_RANGES": number,
        "MARKUP_WORDY_RANGES": markup_wordy,
        "MATH_OPEN_RANGES": math["O"],
        "MATH_CLOSE_RANGES": math["C"],
    }
    text = (
    ...
    text += c_array("XID_START_RANGES", tables["XID_START_RANGES"]) + "\n"
    text += c_ascii_bitset("XID_START_ASCII_BITS", tables["XID_START_RANGES"]) + "\n"
    text += c_array("XID_CONTINUE_RANGES", tables["XID_CONTINUE_RANGES"]) + "\n"
    text += c_ascii_bitset("XID_CONTINUE_ASCII_BITS", tables["XID_CONTINUE_RANGES"]) + "\n"
    text += c_array("NUMBER_RANGES", tables["NUMBER_RANGES"]) + "\n"
    text += c_array("MARKUP_WORDY_RANGES", tables["MARKUP_WORDY_RANGES"]) + "\n"
    text += c_array("MATH_OPEN_RANGES", tables["MATH_OPEN_RANGES"]) + "\n"
    text += c_array("MATH_CLOSE_RANGES", tables["MATH_CLOSE_RANGES"]) + "\n"
    text += "#endif\n"

    verify_required_tables(text)
    js_text = (
        "// Generated by scripts/generate-unicode-tables.py. Do not edit.\n"
        f"// Python regex version: {regex.__version__}\n"
        f"// MathClass source: {display_path(math_path)} ({source_note(math_path)})\n"
        "export const UNICODE_RANGES = Object.freeze({\n"
        f"{js_ranges(tables)}\n"
        "});\n"
    )
    return text, js_text
...
def main() -> None:
    header_text, js_text = generate_outputs()
    write_atomic(OUT, header_text)
    write_atomic(JS_OUT, js_text)
